...
const Cart = ({dispatch , navigate}) => {
  ...
  return (
    <div onClick={e => e.stopPropagation()} className='w-[400px] h-screen bg-black grid grid-rows-10 text-white p-6'>
      <header className='border-b border-gray-500 font-bold text-xl flex items-center row-span-1 h-full justify-between'>
        <span>Your Cart</span>
        <span onClick={() => dispatch(showCart())} className='p-2 cursor-pointer'><AiOutlineClose size={24}/></span>
      </header>
      <section className='row-span-7 flex flex-col gap-3 h-full max-h-full overflow-y-auto py-3'>
        ...
        {currentCart && currentCart?.map(el => (
          <div className='flex justify-between items-center' key={el._id}>
            ...
            <img src={el?.thumbnail} alt="" className='w-16 h-16 object-cover' />
            ...
            <span onClick={() => removeCart(el.product._id , el.color)} className='h-8 w-8 cursor-pointer rounded-full hover:bg-gray-700 flex items-center justify-center'><ImBin size={16}/></span>
          </div>
        ))}
      </section>
